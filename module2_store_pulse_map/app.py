...
def load_and_process(brand_id: str = None):
    ...
    if os.path.exists(REVIEWS_CSV):
        ...
        # New schema: reviews use place_name, join to businesses on name
        # Old schema: reviews use business_id, join directly
        if "business_id" in reviews.columns:
            ...
            # place_name in reviews matches name in businesses
...
def show():
        ...
        for status_val, color in color_map.items():
            ...
            fig.add_trace(go.Scattermapbox(
                ...
                hovertemplate=(
                    ...
                    "📈 vs Peers: <b>%{customdata[2]:+.2f}</b>  💬 Reviews: %{customdata[3]}<extra></extra>"
                ),
                name=status_val,
            ))
    ...
    st.caption("💡 Hover pins for details. Switch to Cluster mode in the sidebar to see density.")
    ...
    disp_cols = [c for c in ["short_label","state","avg_rating","peer_avg","vs_peer","review_count"]
                 if c in filtered.columns]
    bottom = filtered[filtered["status"]=="Below Peer"].sort_values("vs_peer")[disp_cols].head(15).copy()
    ...
    top = filtered[filtered["status"]=="Above Peer"].sort_values("vs_peer", ascending=False)[disp_cols].head(15).copy()
